...
export default function DownloadedContent() {
  ...
  const loadDownloads = async() => {
    if (!user) return
    try {
      const response = await axiosInstance.get(`/downloads/${user?._id}`)
  ...
  if (loading) {
    return <div>Loading Downloaded videos...</div>
  }
  ...
  const handleRemoveFromDownload = async (downloadId: string) => {
    try {
      ...
      console.error("Error removing from Download:", error)
    }
  };
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-600">
          Downloads isn't viewable when signed out.
        </p>
      </div>
    )
  }
